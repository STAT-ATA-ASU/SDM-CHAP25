...
Squaring and summing both sides of \eqref{IDEN} produces
\begin{align}
...
\bar{\bar{y}})(y_{ij} - \bar{y}_i)\label{IDENA}
\end{align}
However, the cross product in \eqref{IDENA} is zero since
\begin{equation*}
...
\begin{equation}\label{SSIDEN}
\sum_{i=1}^{k} \sum_{j=1}^{n_i} (y_{ij} - \bar{\bar{y}})^2 =\sum_{i=1}^k
n_i (\bar{y}_i - \bar{\bar{y}})^2 + \sum_{i=1}^{k} \sum_{j=1}^{n_i}
(y_{ij} - \bar{y}_i)^2
\end{equation}
which says the total variability in the data can be partitioned into two parts. The quantity $\sum_{i=1}^k n_i (\bar{y}_i - \bar{\bar{y}})^2$ measures the difference between the observed treatment means and the grand mean. Specifically, it is a measure of variability due to the treatments and is denoted $SS_\text{Treatment}$ (sum of squares due to treatments).\index{treatments}  The quantity $\sum_{i=1}^{k} \sum_{j=1}^{n_i} (y_{ij} - \bar{y}_i)^2$ measures the differences of observations within a treatment from the treatment mean, which must be due to error and is referred to as $SS_\text{Error}$ (sum of squares due to error).  The quantity on the left-hand side of the equals sign in \eqref{SSIDEN} is called the total sum of squares corrected for the mean and is denoted $SS_\text{Total}$.  The symbolic representation of \eqref{SSIDEN} is
